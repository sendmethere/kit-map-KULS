...
const STORAGE_KEY = 'kit-build-map'
...
const useMapStore = create(
  persist(
    (set, get) => ({
      ...
      clearTeacherMap: () => {
        set({ teacherNodes: [], teacherEdges: [] })
      },

      // ─── Kit slice ────────────────────────────────────────────────────
      kit: { conceptNodes: [], linkLabels: [], frozenEdges: [] },
      ...
      generateKit: () => {
        ...
        if (decompositionMode === 'partial') {
          ...
          frozenEdges = [...teacherEdges].sort(() => Math.random() - 0.5).slice(0, keepCount)
        }
      ...
      checkStudentMap: () => {
        ...
        set({
          feedbackEdges,
          score: {
            correct, missing, excessive, total,
      ...
      clearFeedback: () =>
        set({ feedbackEdges: [], score: { correct: 0, missing: 0, excessive: 0, total: 0, percentage: 0 } }),
    ...
    {
      name: STORAGE_KEY,
      // localStorage에는 teacherNodes/teacherEdges만 저장
      partialize: (state) => ({
        teacherNodes: state.teacherNodes,
        teacherEdges: state.teacherEdges,
      }),
    }
  )
)

export { MAX_NODES, MAX_EDGES }
